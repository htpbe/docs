...
- **Type:** `"consumer_software_origin" | "online_editor_origin" | "scanned_document" | "unverifiable_metadata" | "filled_form_origin" | "fill_sign_origin"` | absent
- **Always Present:** No — only present when `status === "inconclusive"`
- **Description:** Explains why the result is inconclusive.
- **Values:**
  - `"consumer_software_origin"` — the PDF was created by consumer software (Microsoft Word, LibreOffice, Google Docs, etc.) or a freely available HTML-to-PDF renderer. These tools let anyone create a document from scratch, so there is no meaningful "original" to compare against. For a file that _claims_ institutional origin (a payslip, an invoice, a bank statement), this origin is itself a strong signal to route for review — a real payroll or banking system does not distribute documents this way. **If the document will support a financial or compliance decision, verify its content directly with the issuing organisation.**
